...
function verificarCampos(){
   if(nomeCachorro.value != "" && raca.value != ""&& dataNascimento.value != ""&& apelidoCachorro.value != ""
      && porte.value != ""&& pelagem.value != ""&& nomeTutor.value != "" && telefoneTutor.value != "")
      {
         return true;
      }
   return false;
...
function cadastrarCachorro(){
   notify.innerHTML = "";
   if(verificarCampos()){
      const cachorro = criarPet(nomeTutor.value, telefoneTutor.value, nomeCachorro.value, raca.value, dataNascimento.value, 
         perfume.value, apelidoCachorro.value, porte.value, pelagem.value, observacao.value);
      salvarPet(cachorro);
      limparCampos();
